...
import ButtonEdit from '../../../../components/atoms/button/ButtonEdit'
import ButtonTambahNilai from '../../../../components/atoms/button/ButtonTambahNilai'

export const UasGanjil = (props) => {

    // Deklarasi params dan axios
    ...
    const handleEdit = async (val) => {
        ...
        setVisi2('visible')
        setIdSiswa(val)
        const n = nilai.find(({ id_siswa, id_mapel }) => id_siswa == val && id_mapel == params.idMapel)
        ...
        try {
            if (idNilai) {
                ...
                await axios.post('/nilai', {
                    nilai: inputNilai, id_mapel: params.idMapel, id_siswa: idSiswa, jenis_nilai
                })
            }
            setVisi('visible')
            setVisi2('invisible')
                    ...
                    <tbody>
                        { siswa.filter(({ id_kelas }) => id_kelas == idKelas).map((val, index) => (
                            <tr key={ index }>
                                ...
                                <td>
                                    {
                                        nilai.filter(({ id_siswa, id_mapel }) => id_siswa == val.id && id_mapel == params.idMapel).map((value, index) => (
                                            <div key={ index }>
                                                { value.nilai }
                                            </div>
                                        ))
                                ...
                                <td className='d-flex justify-content-around'>
                                    <div key={ index } className='me-5'>
                                        {
                                            nilai.find(({ id_siswa, id_mapel }) => id_siswa == val.id && id_mapel == params.idMapel) == null
